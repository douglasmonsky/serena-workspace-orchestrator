...
public final class LifecycleHttpServer implements AutoCloseable {
    public interface Adapter { List<SafetySnapshot> openProjects(); SafetyDecision freshDecision(String root); boolean close(String root); boolean trust(String root); }
    ...
    private void handle(HttpExchange x) throws IOException { try { if (!("Bearer " + token).equals(x.getRequestHeaders().getFirst("Authorization"))) { reply(x, 401, "{\"error\":\"unauthorized\"}"); return; } if ("GET".equals(x.getRequestMethod()) && "/v1/projects".equals(x.getRequestURI().getPath())) { list(x); return; } if ("GET".equals(x.getRequestMethod()) && "/v1/projects/status".equals(x.getRequestURI().getPath())) { status(x); return; } if ("POST".equals(x.getRequestMethod()) && "/v1/projects/close".equals(x.getRequestURI().getPath())) { closeProject(x); return; } if ("POST".equals(x.getRequestMethod()) && "/v1/projects/trust".equals(x.getRequestURI().getPath())) { trustProject(x); return; } reply(x, 404, "{\"error\":\"not-found\"}"); } catch (RuntimeException ignored) { reply(x, 409, "{\"error\":\"protected\"}"); } finally { x.close(); } }
