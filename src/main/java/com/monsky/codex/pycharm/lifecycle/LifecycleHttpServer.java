...
public final class LifecycleHttpServer implements AutoCloseable {
    public interface Adapter { List<SafetySnapshot> openProjects(); SafetyDecision freshDecision(String root); void close(String root); }
    private final Adapter adapter; private final String token; private final Set<String> closingRoots = new HashSet<>();
    ...
    private void handle(HttpExchange x) throws IOException { try { if (!("Bearer " + token).equals(x.getRequestHeaders().getFirst("Authorization"))) { reply(x, 401, "{\"error\":\"unauthorized\"}"); return; } if ("GET".equals(x.getRequestMethod()) && "/v1/projects".equals(x.getRequestURI().getPath())) { list(x); return; } if ("POST".equals(x.getRequestMethod()) && "/v1/projects/close".equals(x.getRequestURI().getPath())) { closeProject(x); return; } reply(x, 404, "{\"error\":\"not-found\"}"); } catch (RuntimeException ignored) { reply(x, 409, "{\"error\":\"protected\"}"); } finally { x.close(); } }
    ...
    private void closeProject(HttpExchange x) throws IOException { String root = query(x.getRequestURI().getRawQuery(), "root"); if (root == null) { reply(x, 404, "{\"error\":\"not-found\"}"); return; } synchronized (closingRoots) { if (closingRoots.contains(root)) { reply(x, 202, "{\"status\":\"closing\"}"); return; } } if (adapter.openProjects().stream().map(SafetySnapshot::canonicalRoot).noneMatch(root::equals)) { reply(x, 404, "{\"error\":\"not-found\"}"); return; } SafetyDecision d = adapter.freshDecision(root); if (!d.safeToClose()) { String reasons = d.reasons().stream().map(v -> "\"" + json(v) + "\"").reduce((a,b) -> a + "," + b).orElse(""); reply(x, 409, "{\"reasons\":[" + reasons + "]}"); return; } synchronized (closingRoots) { closingRoots.add(root); } adapter.close(root); reply(x, 202, "{\"status\":\"closing\"}"); }
